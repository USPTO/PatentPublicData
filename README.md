...
The official family id is assigned by WIPO, a new id is given if an new <a href="https://en.wikipedia.org/wiki/Patent_Cooperation_Treaty">PCT application</a> does not belong to a family. EPO utilizes the  Family Id to help derive the initial CPC Classification given to an application.  The family id is not included in a patent's bibliography or within USPTO's bulk files.
