...
public class WordUtil {
	...
	public static String removeSurrounding(final String token) {
		char[] chars = token.toCharArray();
		if (surroundPairs.containsKey(chars[0])) {
			if (chars[chars.length - 1] == surroundPairs.get(chars[0])) {
				return token.substring(1, chars.length - 1);
			}
		}
		return token;
	}
